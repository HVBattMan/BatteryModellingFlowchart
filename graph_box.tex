...
\begin{tikzpicture}
	...
	\node [
		...
		label=below:{
			EPPM \\[\labellinespace]
			{\tiny Electrolyte} \\[\labellinespace]
			{\tiny with} \\[\labellinespace]
	...
	\node [
		modelpoint,
		label=below:{ ECCM \\[\labellinespace] 
		  {\tiny Electrolyte} \\[\labellinespace]
		  {\tiny with Current} \\[\labellinespace]
		  {\tiny Collectors} \\[\labellinespace]
		  {\tiny Model}
		}
	]
	...
	\node [
		modelpoint,
		label=below:{REM \\[\labellinespace]
	...
	%lifp
	\node (lifp_start) at ($(lead_acid.south) + (0,0cm)$) {};
	\draw [line width=1mm]
