...
\begin{tikzpicture}
	...
	\node [physics] at (single_particle.south east) {Narrow \\ distribution};
	%
	...
	\node [physics] at (0D_micro.south east) {Fast particle \\ diffusion};
	% axis
	...
	%
	\node [axisbox, below = \VERTFIGSEP of 0plus1D, anchor=north](0plus0D)
		{\includegraphics[width=\FIGWIDTH, height=\FIGHEIGHT]{figures/0D}};
	\node [dimensions, \macrocolor] at (0plus0D.north east) {$0\xo0$};
	\node [physics] at (0plus0D.south east) {Fast electrolyte \\ diffusion};
	% axis
	% \node [above right = 0cm and 2.5cm of 2plus1D](y_axis_start) {};
	% \node [below right = 0cm and 2.5cm of 0plus0D](y_axis_end) {};
	\node (y_axis_end) at (0plus0D.south -| axis_origin) {};
	...
	% Results
	\node [axisbox, anchor=north west] (2D_results)
		at (2plus1D.north east -| particle_distribution.south west)
		{\includegraphics[width=45cm, height=15cm]{example-image-a}};
	% Arrows
	...
	\node [
		...
		(SPM) at (0plus0D -| single_particle) {};
	\node [
		modelpoint,
		label=right:{
			\includedimensions{$\macrotext{0\xo0}\black{\xo}\microtext{1}$ (}
		...
		(SPMe) at ($(SPM)!0.5!(DFN)$) {};
	% Results
	\node [axisbox, anchor=north west] (SPM_results)
		at (2plusbar1D.east -| particle_distribution.south west)
		{\includegraphics[width=25cm, height=25cm]{example-image-a}};
	% Arrows
	...
	\node [
		...
		(DPM) at (0plus0D -| particle_distribution) {};
	\node [
		modelpoint,
		label=right:{
			\includedimensions{$\macrotext{0\xo0}\black{\xo}\microtext{1}$ (}
